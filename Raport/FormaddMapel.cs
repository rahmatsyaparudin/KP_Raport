...
namespace Raport
{
    public partial class FormAddMapel : Form
    {
        ...
        public string getKodeKelas, kodeIdGuru, getTahun;
        ...
        private void schedule_grid_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if ((e.RowIndex >= 0) && (e.RowIndex != -1))
                {
                    ...
                    cond = "status = 'Aktif' AND tahun_ajaran = '" + getTahun + "' AND kode_mapel = '" + kodeMapel + "'";
        ...
        public void create_schedule()
        {
            try
            {
                ...
                if (passIdGuru != null)
                {
                    this.field = "kode_mapel as 'Kode Mapel', kategori_mapel as 'Kategori Mapel', mata_pelajaran as 'Mata Pelajaran', jam_pelajaran as 'JP'";
                    this.table = "mapel";
                    this.cond = "status_mapel = 'Aktif' AND kode_mapel NOT IN (SELECT kode_mapel FROM detailmapelguru WHERE id_guru = '" + passIdGuru +
                                "' AND tahun_ajaran = '" + getTahun + "' AND status = 'Aktif') ORDER BY kategori_mapel";
                    DataTable tabel = db.GetDataTable(field, table, cond);
                    this.schedule_grid.DataSource = tabel;
                }
                ...
                cmb.ReadOnly = false; cmb.MaxDropDownItems = 5;
        ...
        private void create_btn_Click(object sender, EventArgs e)
        {
            try
            {
                kodeKelas = getKodeKelas;
                kodeIdGuru = passIdGuru;
                char notif; notif = 'A';

                foreach (DataGridViewRow row in schedule_grid.Rows)
                {
                    kodeMapel = row.Cells[1].Value.ToString();
                    if (kodeKelas != null)
                    {
                    ...
                    if (passIdGuru != null)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value) == true)
                        {
                            ...
                            string status_yes = "SELECT kode_mapel as 'Kode' from mapel where kode_mapel = '" +
                                                ...
                                                kodeMapel + "' AND id_guru = '" + kodeIdGuru + "' AND tahun_ajaran = '" + getTahun + "' )";
                            myConn.Open();
                            myComm = new MySqlCommand(status_yes, myConn);
                            myReader = myComm.ExecuteReader();
                            while (myReader.Read())
                            {
                                string getMapel = myReader.GetString("Kode");
                                this.table = "detailmapelguru";
                                this.field = "DEFAULT, '" + kodeIdGuru + "', '" + kodeMapel + "', '" + getTahun +
                                             "', DEFAULT";
                                db.insertData(table, field);
                            }
                            myConn.Close();
